...
class TUMDataset(BaseDataset):
    def __init__(self, cfg, basedir, align=True, trainskip=1,
        ...
        self.color_paths, self.depth_paths, self.poses, self.times = self.loadtum(
    ...
    def loadtum(self, datapath, frame_rate=-1):
        ...
        images, poses, depths, intrinsics, times = [], [], [], []
        inv_pose = None
        for ix in indicies:
            (i, j, k, t) = associations[ix]
            ...
            poses += [c2w]
            times += [t]
        times = torch.FloatTensor(np.array(times, dtype=float))

        return images, depths, poses, times
